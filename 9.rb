class IntcodeComputer
  ...
  def compute(inputs)
    ...
    while true do
      ...
      when "03"
        args = get_args(1)
        set_value(inputs.shift, args[0], modes[0])
      when "04"
        args = get_args(1)
      ...
      when "07"
        ...
        if get_value(args[0], modes[0]) < get_value(args[1], modes[1])
          set_value(1, args[2], modes[2])
        else
          set_value(0, args[2], modes[2])
        end
      when "08"
        args = get_args(3)
      ...
      when "09"
        args = get_args(1)
        @relative_base = @relative_base + get_value(args[0], modes[0])
      end
  ...
  def get_value(param, mode)
    ...
    when ImmediateMode
      param
    when RelativeMode
      @hash[@relative_base + param] || 0
    end
  end

  def set_value(val, param, mode)
    case mode
    when PositionMode
      @hash[param] = val
    when RelativeMode
      @hash[@relative_base + param] = val
    end
  end

...
def calc(w1, w2)
  ...
  ints = find_intersections(p1, p2)
  closest = ints.first
  c_sum = closest.x.abs + closest.y.abs

  ints.each do |int|
    sum = int.x.abs + int.y.abs
    if sum < c_sum
      closest = int
      c_sum = sum
    end
...
def generate_path(w)
  path = [OpenStruct.new(x: 0, y: 0, steps: 0)]

  w.split(",").each do |instruction|
    current_point = path[-1]
    direction = instruction[0]
    num = instruction[1..-1].to_i
    new_point = OpenStruct.new(steps: current_point.steps + num)
    case direction
    when "U"
      new_point.x = current_point.x
      new_point.y = current_point.y + num
    when "D"
      new_point.x = current_point.x
      new_point.y = current_point.y - num
    when "L"
      new_point.x = current_point.x - num
...
def find_intersections(path1, path2)
  ints = []
  for i in 2..path1.length-1 do
    orig1 = path1[i-1]
    p1 = path1[i]

    for ii in 2..path2.length-1 do
      orig2 = path2[ii-1]
      p2 = path2[ii]

      if intercepts_x?(orig1, p1, p2) && intercepts_y?(orig2, p2, p1)
        total_steps = orig1.steps + orig2.steps
        if orig2.y < p2.y # goin' up
          total_steps += orig1.y - orig2.y
        else # goin' down
          total_steps += orig2.y - orig1.y
        end

        if orig1.x < p1.x # goin' up
          total_steps += orig2.x - orig1.x
        else
          total_steps += orig1.x - orig2.x
        end
        ints << OpenStruct.new(x: p2.x, y: p1.y, steps: total_steps)
      elsif intercepts_x?(orig2, p2, p1) && intercepts_y?(orig1, p1, p2)
        total_steps = orig1.steps + orig2.steps
        if orig1.y < p1.y # goin' up
          total_steps += orig2.y - orig1.y
        else # goin' down
          total_steps += orig1.y - orig2.y
        end
        ...
        ints << OpenStruct.new(x: p1.x, y: p2.y, steps: total_steps)
      end
    end
  end
  ints
...
def intercepts_x?(orig1, p1, p2)
  sorted = [orig1.x, p1.x].sort
  sorted.first <= p2.x && p2.x <= sorted.last
end

def intercepts_y?(orig1, p1, p2)
  sorted = [orig1.y, p1.y].sort
  sorted.first <= p2.y && p2.y <= sorted.last
end
